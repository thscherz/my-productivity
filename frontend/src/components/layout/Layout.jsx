...
export default function Layout({
  ...
  onCreateTask,
  onLogout,
  hideCompletedWP = true,
  onToggleHideCompletedWP,
}) {
    ...
    <div className="flex h-screen flex-col" style={{ backgroundColor: "#F5F4F2" }}>
      ...
      <header className="z-20 flex items-center gap-3 px-4 py-2.5 shadow-md" style={{ backgroundColor: "#1E3A5F" }}>
        ...
        <div className="hidden md:flex items-center gap-2 flex-1">
          ...
          {/* Toggle: Abgeschlossene Arbeitspakete ausblenden
              Aktiv (hideCompletedWP=true)  = Erledigte ausgeblendet → Button ausgefüllt + durchgestrichenes Auge
              Inaktiv (hideCompletedWP=false) = Erledigte sichtbar    → Button transparent + offenes Auge */}
          {onToggleHideCompletedWP && (
            <button
              onClick={onToggleHideCompletedWP}
              title={hideCompletedWP
                ? "Erledigte WPs ausgeblendet — klicken zum Einblenden"
                : "Erledigte WPs werden angezeigt — klicken zum Ausblenden"}
              className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors border ${
                hideCompletedWP
                  ? "border-white/50 bg-white/15 text-white hover:bg-white/20"
                  : "border-white/20 text-white/50 hover:text-white/80 hover:border-white/40"
              }`}
            >
              {/* Auge-Symbol: durchgestrichen = aktiv (ausgeblendet), offen = inaktiv (sichtbar) */}
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                {hideCompletedWP ? (
                  // Durchgestrichenes Auge: Erledigte sind ausgeblendet
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                ) : (
                  // Offenes Auge: Erledigte sind sichtbar
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                    d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                )}
              </svg>
              <span className="hidden lg:inline">Erledigte WPs</span>
            </button>
          )}
        </div>
      ...
      <div className="md:hidden border-b border-gray-200 bg-white px-4 py-2 flex gap-2">
        ...
        {/* Toggle: Abgeschlossene Arbeitspakete ausblenden (Mobile)
            Aktiv = ausgefüllt blau + durchgestrichenes Auge, Inaktiv = grau + offenes Auge */}
        {onToggleHideCompletedWP && (
          <button
            onClick={onToggleHideCompletedWP}
            title={hideCompletedWP
          ...
          >
            <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              {hideCompletedWP ? (
                // Durchgestrichenes Auge: Erledigte sind ausgeblendet (aktiver Zustand)
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                  d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
              ) : (
                // Offenes Auge: Erledigte sind sichtbar (inaktiver Zustand)
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                  d="M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
              )}
            </svg>
